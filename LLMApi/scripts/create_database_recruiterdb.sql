...
IF EXISTS (SELECT name FROM sys.databases WHERE name = 'RecruiterDb')
BEGIN
    ALTER DATABASE RecruiterDb SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE RecruiterDb;
    PRINT 'Database RecruiterDb dropped successfully.';
END
GO

CREATE DATABASE RecruiterDb;
GO
