...
export default function InterviewConfigurationDetailsPage() {
  const params = useParams();
  const api = useApi();
  const [configuration, setConfiguration] = useState<InterviewConfigurationWithPrompts | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchConfiguration = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await api.get(`/InterviewConfiguration/${name}/${version}/with-prompts`);        
