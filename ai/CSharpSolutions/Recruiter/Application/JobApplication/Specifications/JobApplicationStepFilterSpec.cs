...
public sealed class JobApplicationStepFilterSpec : Specification<Domain.Models.JobApplicationStep>
{
    public JobApplicationStepFilterSpec(JobApplicationStepListQueryDto query)
    {
        // Apply filters
        ...
        if (!string.IsNullOrWhiteSpace(query.SortBy))
        {
            switch (query.SortBy.ToLowerInvariant())
            {
                case "jobapplicationid":
                    Query.OrderBy(jas => jas.JobApplicationId, query.SortDescending);
                    break;
                case "jobpoststepname":
                    Query.OrderBy(jas => jas.JobPostStepName, query.SortDescending);
                    break;
                case "status":
                    Query.OrderBy(jas => jas.Status, query.SortDescending);
                    break;
                case "stepnumber":
                    Query.OrderBy(jas => jas.StepNumber, query.SortDescending);
                    break;
                case "startedat":
                    Query.OrderBy(jas => jas.StartedAt, query.SortDescending);
                    break;
                case "completedat":
                    Query.OrderBy(jas => jas.CompletedAt, query.SortDescending);
                    break;
                case "createdat":
                default:
                    Query.OrderBy(jas => jas.CreatedAt, query.SortDescending);
                    break;
            }
        }
        else
